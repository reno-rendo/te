  ...
  if (isLoading) {
    return (
      <div className="space-y-8">
        <div className="aspect-[21/9] rounded-xl animate-blink" style={{ background: 'hsl(var(--muted))' }} />
  ...
  return (
    <div className="space-y-8">
      ...
      {bookGroups.map((group, index) => (
        <section key={index}>
          <h2 className="section-title section-title-accent mb-5">
            {group.title}
          </h2>
...
function SectionSkeleton() {
  return (
    <div>
      <div className="h-6 w-32 rounded-xl animate-blink mb-4" style={{ background: 'hsl(var(--muted))' }} />
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 md:gap-4">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i}>
            <div className="aspect-[2/3] rounded-xl animate-blink" style={{ background: 'hsl(var(--muted))' }} />
            <div className="mt-1.5 h-2.5 rounded animate-blink" style={{ background: 'hsl(var(--muted))' }} />
          </div>
        ))}
      </div>
    </div>
